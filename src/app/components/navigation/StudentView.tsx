...
import { Search, List, Filter, X, MapPin, Lightbulb } from 'lucide-react';
...
export default function StudentView({ onGoHome }: StudentViewProps) {
  ...
  // Suggest POI form state
  const [suggestName, setSuggestName] = useState('');
  const [suggestCategory, setSuggestCategory] = useState('');
  const [suggestCampus, setSuggestCampus] = useState('');
  const [suggestFloor, setSuggestFloor] = useState('');
  ...
  const handleSuggestSubmit = () => {
    if (!suggestName || !suggestCategory || !suggestCampus || !suggestFloor) {
      setShowErrors(true);
      return;
    }
  ...
  return (
    <div className="size-full relative">
      ...
      {/* Action Buttons */}
      {!showPOIPanel && !showSuggestPanel && !selectedPOI && (
      ...
      {showSuggestPanel && (
        <div className="fixed inset-x-4 top-1/2 -translate-y-1/2 z-[1001] mx-auto max-w-md">
          <div className="bg-white rounded-lg shadow-xl border border-slate-200 shadow-[0_4px_4px_rgba(0,0,0,0.28)] p-6">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl text-[rgb(0,50,130)]">Sugerir POI</h3>
              <button onClick={() => {
                setShowSuggestPanel(false);
                setShowErrors(false);
              }}>
            ...
            {showSuccess && (
              <div className="mb-4 p-3 bg-green-100 border border-green-400 rounded-lg">
                <p className="text-green-700 text-center">¡POI sugerido exitosamente!</p>
              </div>
            )}
            ...
            <div className="flex flex-col gap-4">
              ...
              <div>
                <select
                  value={suggestCampus}
                  onChange={(e) => setSuggestCampus(e.target.value)}
                  className={`w-full px-4 py-3 border-2 rounded-lg focus:outline-none focus:ring-2 ${
                    showErrors && !suggestCampus
                      ? 'border-red-500 focus:ring-red-500'
                      : 'border-[rgb(0,50,130)] focus:ring-[#C8102E]'
                  }`}
                >
              ...
              <div>
                <select
                  value={suggestFloor}
                  onChange={(e) => setSuggestFloor(e.target.value)}
                  className={`w-full px-4 py-3 border-2 rounded-lg focus:outline-none focus:ring-2 ${
                    showErrors && !suggestFloor
                      ? 'border-red-500 focus:ring-red-500'
                      : 'border-[rgb(0,50,130)] focus:ring-[#C8102E]'
                  }`}
                >
              ...
              <button
                ...
                className="w-full px-6 py-3 bg-[#10B981] text-white rounded-lg hover:bg-[#059669] transition-colors"
              >
                Confirmar
              </button>
            </div>
